...
describe('The botStateMiddleware', () => {
  ...
  beforeEach(() => {
    ...
    botState.setBotData(channelId, conversation.conversationId, user.id, { eTag: '', data: { state: 'none' } });
  ...
  it('should set conversation data', () => {
    ...
    expect(sendSpy).toHaveBeenCalledWith(HttpStatus.OK, { data: req.body.data, eTag: '123456' });
    expect(emulator.facilities.botState.getBotData(channelId, conversation.conversationId, user.id)).toEqual({

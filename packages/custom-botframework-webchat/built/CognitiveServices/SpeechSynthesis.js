...
class CognitiveServicesHelper {
    constructor(props) {
        ...
        if (props.subscriptionKey) {
            this._tokenCallback = (id) => this.fetchSpeechToken(id);
            this._tokenExpiredCallback = (id) => this.fetchSpeechToken(id);
        }
    ...
    fetchSpeechData(text, locale, synthesisProperties) {
        const SSML = this.makeSSML(text, locale, synthesisProperties);
        const cbAfterToken = (token) => {
    ...
    makeSSML(text, locale, synthesisProperties) {
    ...
    processSSML(ssml, synthesisProperties) {
        ...
        if (!processDone) {
            ...
            const voiceNode = dom.createElement("voice");
            const attribute = dom.createAttribute("name");
            attribute.value = (synthesisProperties && synthesisProperties.voiceName) || this.fetchVoiceName(locale, gender);
    ...
    fetchSpeechToken(apiKey) {
        const optionalHeaders = [{ name: "Ocp-Apim-Subscription-Key", value: apiKey },
    ...
    makeHttpCall(actionType, url, isArrayBuffer = false, optionalHeaders, dataToSend) {
